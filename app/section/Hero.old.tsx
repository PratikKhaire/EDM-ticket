...
export const Hero = () => {
  ...
  const [centerCardIndex, setCenterCardIndex] = useState(2); // Center card is index 2
  const cardPositions = useRef<number[]>([0, 1, 2, 3, 4]); // Track which card is at which position
  ...
  useGSAP(() => {
    ...
    cardsRef.current.forEach((card, index) => {
      if (card) {
        // Initially hide all cards except center (index 2)
        if (index === 2) {
          // Center card visible from start
          gsap.set(card, {
            x: 0,
            z: 50,
            rotationY: 0,
        ...
        // Add hover listeners for card interactions
        const handleCardMouseEnter = () => {
          if (!isRevealed && index === 2) {
            // Hovering on center card - REVEAL ALL CARDS
            setIsRevealed(true);
            
            // Animate all cards to their positions with stagger
            cardsRef.current.forEach((c, i) => {
              if (c && i !== 2) {
                gsap.to(c, {
                  x: initialPositions[i].x,
                  z: initialPositions[i].z,
                  rotationY: initialPositions[i].rotationY,
                  scale: 1,
                  opacity: 1,
                  duration: 0.8,
                  delay: Math.abs(i - 2) * 0.1, // Stagger from center
                  ease: "back.out(1.2)"
                });
              }
            });
          } else if (isRevealed) {
            // After reveal, hover brings card to center with swap
            
            if (index !== centerCardIndex) {
              // SWAP: Move hovered card to center
              const oldCenterIndex = centerCardIndex;
              
              // Find current positions in the visual layout
              const hoveredCardCurrentPos = cardPositions.current.indexOf(index);
              const centerCardCurrentPos = cardPositions.current.indexOf(oldCenterIndex);
              
              // Swap positions in tracking array
              cardPositions.current[hoveredCardCurrentPos] = oldCenterIndex;
              cardPositions.current[centerCardCurrentPos] = index;
              
              // Update center card state
              setCenterCardIndex(index);
              
              // Animate hovered card to center with pop
              gsap.to(card, {
                x: 0,
                z: 200,
                rotationY: 0,
              ...
              // Animate previous center card to where hovered card was
              const prevCenterCard = cardsRef.current[oldCenterIndex];
              if (prevCenterCard) {
                gsap.to(prevCenterCard, {
                  x: initialPositions[hoveredCardCurrentPos].x,
                  z: initialPositions[hoveredCardCurrentPos].z,
                  rotationY: initialPositions[hoveredCardCurrentPos].rotationY,
                  scale: 1,
                  duration: 0.6,
                  ease: "power2.out"
                });
              ...
              // Hovering on current center - just pop it up
              gsap.to(card, {
                z: 200,
                scale: 1.15,
                duration: 0.5,
        ...
        const handleCardMouseLeave = () => {
          if (isRevealed) {
            // Return card to its current assigned position
            const currentVisualPosition = cardPositions.current.indexOf(index);
            
            gsap.to(card, {
              x: initialPositions[currentVisualPosition].x,
              z: initialPositions[currentVisualPosition].z,
              rotationY: initialPositions[currentVisualPosition].rotationY,
              scale: 1,
              duration: 0.5,
              ease: "power2.out",
              overwrite: true
    ...
    <div className="max-w-[1400px] mx-auto w-full px-8 md:px-16 lg:px-24 xl:px-32 py-16 md:py-20">
      <div className="md:flex items-center justify-between gap-12 lg:gap-16">
        ...
        <div className="hidden md:flex md:order-2 md:w-[660px] md:h-[448px] relative items-center justify-center flex-shrink-0" style={{ background: 'transparent' }}>
          ...
          {/* Hover Hint - Shows before reveal */}
          {!isRevealed && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
              <div className="bg-black/60 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm font-medium animate-pulse">
                Hover to explore more events ✨
              </div>
            </div>
          )}
